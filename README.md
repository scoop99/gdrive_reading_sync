...
> **플러그인 게시판의 자동 설치로는 설치되지 않습니다.** 게시판은 `subprocess`
> import 를 금지 패턴으로 차단하는데, 이 플러그인은 rclone 을 실행하는 것이 존재
> 이유라 `subprocess` 를 뺄 수 없습니다. **폴더를 직접 복사**해 주세요.
>
> 최초 설치만 수동이고, 그 뒤 버전 업데이트는 환경설정의 업데이트 버튼으로 됩니다.
