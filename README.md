...
- **고아 파일 격리 (v0.4.0~0.4.1)** — 구드에서 파일 이름만 바뀌면 파일 ID 가 달라
  ...
  화면 `정리 대기` 탭에서 사람이 선택합니다. `_trash` 는 보존기간이 지나야 정리됩니다.

  **화면만 보고 판단할 수 있습니다** — 폴더 단위로 접혀 보이고, 펼치면
  `옛 파일 · 옛 크기 · 새 파일 · 새 크기 · 크기 차이` 가 나란히 나옵니다(`-3.0 MB` 처럼).
  같은 `NN권` 의 새 파일을 1순위로 짝지어 주며, 폴더에 고아가 아닌 다른 파일도 보여 줍니다.
  `이 폴더 격리`/`이 폴더 보존` 으로 폴더 단위 일괄 처리하고, 격리(삭제 아님, 30일 복구)와
  보존의 의미는 버튼 옆에 보이는 문구로 설명합니다
- **병렬 전송** — 기본 5. 실제 바이트를 옮기는 작업만 병렬로 돌리고 이름변경 판정은
  직렬이라 경합이 없음
- **목록 화면** — 서버측 페이징 · 상태/유형/결과 필터 · 검색 · 정렬, 일괄 재처리,
  이력 자동 정리
- **날짜별 로그** — 로컬 시각 + UTC 오프셋으로 기록하고 로컬 자정마다 회전.
  보존 일수 설정 가능. 변경이 없는 폴링은 **한 시간에 한 줄로 접어** 기록하므로
  의미 있는 줄이 묻히지 않습니다(실측 90~98% 감소). 작업·오류는 언제나 그대로 남습니다
